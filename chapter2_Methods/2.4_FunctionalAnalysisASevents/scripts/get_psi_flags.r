...
#load event counts
load(file.path(cts_dir,cancer,"trex/event_counts",paste0(event,".eventCounts.RData")),verbose = F)

analysis<-"condition"
res_dir<-file.path(cts_dir,cancer,"trex/coefs_condition")
...
cond <- lapply(res.files,function(file,...){
                 res <- suppressWarnings(fread(file.path(res_dir,file),data.table=FALSE,stringsAsFactors = F,nThread = 4))%>%
...
write.table(psi.stats,file=paste0(out_dir,"/",cancer,"_",event,"_psi",min.psi,"_flags.tsv"),row.names=F,col.names=T,sep="\t",quote=FALSE)
